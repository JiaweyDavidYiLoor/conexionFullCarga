...
import com.st.integracion.network.FullCarga.ClaroChannelHandler;
import com.st.integracion.network.FullCarga.ClaroTrbListener;
...
import com.st.integracion.ws.FullCarga.TipoProducto;
...
public class ServicioFullCarga implements ServicioConeccion, Job {
	...
	private ConcurrentMap<TransaccionFullCarga, ClaroTrbListener> pendientes;
	...
	@Inject private VariablesFullCargaLocal variables;
    private static final String productoCupon = "CUPON";
    private static final String COD_ANULACION_CONTRATO = "2";
    private static final String CAUSA_ANULACION = "PGE";
	...
	{ 
		...
		this.pendientes=new ConcurrentSkipListMap<TransaccionFullCarga, ClaroTrbListener>();
		if(periodoEspera>0)
	...
	throws JobExecutionException
	{
		try
		{
			...
			AdminFullCarga adminInteragua = (AdminFullCarga)RegistroServicios.registroInstance().getAdminConeccion(tx.getCodigoProveedorProducto());
			ServicioFullCarga srv = (ServicioFullCarga)RegistroServicios.registroInstance().getServicioConeccion(tx.getCodigoProveedorProducto());
	...
	@Override public void recargar(Transaccion tran) {
		...
		TransaccionFullCarga tx=(TransaccionFullCarga) tran;
			...
			{
	        	 log.info("SchedulerException-ServicioFullCarga-recargar: no se pudo instanciar con JobDetail && Trigger..!");
	...
	@Override public void consultarRecarga(Transaccion tran)
	{
		TransaccionFullCarga tx=this.prepararTransaccion(tran, TipoOperacion.CONSULTA_RX);
		try {
			List<TransaccionFullCarga> ticketsPendientes=this.pendientesPorTicket(tran.getNumTransaccion(), TipoOperacion.RECARGA , TipoOperacion.CONSULTA_RX);
			for(TransaccionFullCarga t:ticketsPendientes) {
				ClaroTrbListener listener=(ClaroTrbListener) this.pendientes.get(t);
		...
		try
		{
			...
			log.info("ScheluderException-ServicioFullCarga-consultarRecarga: anulacion no se pudo instanciar con GftJobDetail && GftTrigger.. !");
			e.printStackTrace();
		}
	}

	@Override public void comprarPin(Transaccion tran) {}

	@Override public void consultarCompraPin(Transaccion tran) {}
		...
		TimerTask conTask=new TimerTask() {
			...
			public void run() {
				try 
				{
					...
					log.warn("Exception-ServicioFullCarga-realizarPendientes: TransaccionTelebillete Erronea saldra en el log " , e);
				}
			}
		};
			...
			case RECARGA:
				...
				else
				{
					try {
						...
						log.info("Exception-ServicioFullCarga-realizarPendientesAction: Transaccion No Ticket: "+tx.getNumTransaccion() + " no se ha enviado a el proveedor y no se ha guardado en BASE DATOS");
					}
				}
				break;
			}
	...
	public void cancelarPendientes() {
		for(TransaccionFullCarga tx:this.pendientes.keySet()) {
			ClaroTrbListener l=this.pendientes.remove(tx);
			((TimerTask) l).cancel();
		}
	}

	public void transaccionEnviadaProveedor(ClaroTrbListener l) {
	...
	public void transaccionNoEnviadaProveedor(TransaccionFullCarga txR) 
	{
		...
		{
			String msg=String.format("Exception-ServicioFullCarga-transaccionNoEnviadaProveedor: no sera sacada de pendientes la transaccion %s ", txR);
			log.info(msg, e);
			return;
		}
	}

	public void respuestaRecibidaProveedor(ClaroTrbListener l)
	{
		...
		try 
		{
			...
			String msg=String.format("Exception-ServicioFullCarga-respuestaRecibidaProveedor: no sera sacada de pendientes la transaccion %s ", tx);
			log.info(msg, e);
			return;
		}
	...
	private Trigger getTrigger(TransaccionFullCarga tx)
	{
		...
		if(cantReqRealizados == null ){
			log.info("ServicioEasyCash-getTrigger: cantRequerimientosRealizados fue null : tx.getCantReqRealizados =  null");
			cantReqRealizados = 0;
		}
		else
		{
			try{cantReqRealizados = cantReqRealizados+1;}
			catch(Exception e){cantReqRealizados = 0;}
		}
		Integer cantidadCopia = cantReqRealizados;
		tx.setCantReqRealizados(cantReqRealizados);
		//log.info("Trigger preparado con Cantidad Requerimientos Realizados :"+ tx.getCantReqRealizados());
	...
	/**
	 * ADICIONA LA TRANSACCION RECIEN ENVIADA COMO PENDIENTE (Y SU CORRESPONDIENTE LISTENER) Y ADICIONA EL LISTENER AL HANDLER
	 * @param l
	 */
	private void addAndRunPendiente(ClaroTrbListener l) {
		this.pendientes.putIfAbsent(l.getTx(), l);
		ClaroChannelHandler.addChannelListener(l);
		if(this.pendientes.size()>0)
		log.info(String.format("ServicioFullCarga-addAndRunPendiente: actualmente hay %d transacciones pendientes", this.pendientes.size()));
		if(l.getTx().getTipoOperacion().compareTo(TipoOperacion.ANULACION_CMP_PIN_INTERNET)==0)
		{
			...
			log.info(String.format("ServicioFullCarga-addAndRunPendiente: ha sido adicionada y programada por "+this.periodoEsperaReverso/1000+" segundos la transaccion %s ", l.getTx()));
		}
		else
		{
			this.timer.scheduleAtFixedRate(l, this.periodoEspera, this.periodoEspera);
			log.info(String.format("ServicioFullCarga-addAndRunPendiente: ha sido adicionada y programada por "+this.periodoEspera/1000+" segundos la transaccion %s ", l.getTx()));
		}
				
	}

	private void removeAndCancelPendiente(ClaroTrbListener l) {
		ClaroTrbListener listener=this.pendientes.remove(l.getTx());
		ClaroChannelHandler.removeChannelListener(listener);
		if(this.pendientes.size()>0)
		log.info(String.format("ServicioFullCarga-removeAndCancelPendiente: actualmente hay %d transacciones pendientes", this.pendientes.size()));
		try{((TimerTask) listener).cancel();}catch(NullPointerException npe){};
		log.info(String.format("ServicioFullCarga-removeAndCancelPendiente: la transaccion %s ha sido removida y cancelada", l.getTx()));
	}

	private void writeTransaccion(final TransaccionFullCarga tx,final AdminFullCarga adminInteragua) throws JAXBException 
	{
		...
		do
		{
			...
			ServicioFullCarga sr = (ServicioFullCarga) this.getRegistro().getServicioConeccion(tx.getCodigoProveedorProducto());
			...
			InfoEmpresa infoEmpresa = (InfoEmpresa) util.convertirXmlToObjeto(tx.getParametrosInfoEmpresa(), InfoEmpresa.class);
		...
		else
		{
			if(channel.isOpen())
			{
	            ...
	            f.addListener(new ClaroTrbListener(this,tx));
	        }
			else
			{
				String descripcionError=String.format("ServicioFullCarga-writeTransaccion: error channel no connected");
				tx.setDescripcion(descripcionError);
				transaccionNoEnviadaProveedor(tx);
				channel.close();
				return;
	...
	private TransaccionFullCarga prepararTransaccion(Transaccion tran, TipoOperacion op) 
	{
		...
		try
		{
			...
			ServicioFullCarga servicioInteragua = (ServicioFullCarga)this.registro.getServicioConeccion(tx.getCodigoProveedorProducto());
			...
			try
			{
				switch (op)
				{
					case COMPRAR_PIN_INTERNET:																				
						String parametrosXmlIN = tx.getParametrosXmlInTransaccion();
						//String [] parametrosXmlInArray = parametrosXmlIN.split("SEPARADORST");
						//String parametrosRespuesta = parametrosXmlInArray[0];
						//String xmlConsulta = parametrosXmlInArray[1];
						
						//ParametrosRespuesta pr = (ParametrosRespuesta) Utilitaria.convertirXmlToObjeto(parametrosRespuesta, ParametrosRespuesta.class);
						
						BigDecimal comision = new BigDecimal(0.57);						                             					
						BigDecimal valorCobrado = tx.getValorContable().subtract(comision);						
						
						//tx.setParametrosRespuesta(tx.getParametrosRespuesta());
						tx.setFechaEnvioCompraPinInternet(new Date());
																									
						break;
					case ANULACION_CMP_PIN_INTERNET:
																								
						tx.setFechaEnvioAnulacionCompraPinInternet(new Date());
						tx.setParametrosRespuesta(tx.getParametrosRespuesta());
					...
					default:
						log.info(String.format("ServicioFullCarga-prepararTransaccion: %s no se puede preparar la operacion de la tx ", tx.getNumeroTransaccion()));
						return null;
				}
				log.info("ServicioFullCarga-prepararTransaccion: "+tx.getNumeroTransaccion()+"transaccion preparada para enviarse a procesar: ");
				
				this.registro.getAdminTransaccion(tx.getCodigoProveedorProducto()).transaccionEnviada(tx);
			} catch (TransaccionException e) {
				log.info("Exception-ServicioFullCarga-prepararTransaccion: Error al marcar la transaccion como enviada", e);
				return null;
			}
		}catch (Exception e) {
	...
	@Override public void comprarPinInternet(Transaccion tran) {
	...
	@Override public void consultarCompraPinInternet(Transaccion tran){}

	@Override public void anularCompraPinInternet(Transaccion tran) 
	{
		this.jitInit(tran.getCodigoProveedorProducto());
		...
		try {
			List<TransaccionFullCarga> ticketsPendientes=this.pendientesPorTicket(tran.getNumTransaccion(), TipoOperacion.COMPRAR_PIN_INTERNET,TipoOperacion.ANULACION_CMP_PIN_INTERNET);
			for(TransaccionFullCarga t:ticketsPendientes) {
				ClaroTrbListener listener=(ClaroTrbListener) this.pendientes.get(t);
				this.removeAndCancelPendiente(listener);
			}
		...
		try
		{
			...
			log.info("ScheluderException-ServicioFullCarga-anularCompraPinInternet: No se pudo instanciar con GftJobDetail && GftTrigger.. !");
			e.printStackTrace();
		}
	}

	@Override public void consultarAnulacionCompraPinInternet(Transaccion tran) 	{}

	@Override public void consultarAnulacion(Transaccion tran) {}

	@Override public void activar(Transaccion tran){}

	@Override public void desactivar(Transaccion tran) {}

	@Override public void anularCompraPin(Transaccion tran) {}

	@Override public void consultarActivacion(Transaccion tx) {}

	@Override public void consultarDesactivacion(Transaccion tx) {}

	@Override public void anularCompraPinInternacional(Transaccion tx) {}

	@Override public void anularRecargaInternacionalTarjeta(Transaccion tx) {}

	@Override public void anularRecargaInternacionalTelefono(Transaccion tx) {}

	@Override public void comprarPinInternacional(Transaccion tx) {}

	@Override public void consultarAnulacionCompraPin(Transaccion tx) {}

	@Override public void consultarAnulacionCompraPinInternacional(Transaccion tx) {}

	@Override public void consultarAnulacionRecargaInternacionalTarjeta(Transaccion tx) {}

	@Override public void consultarAnulacionRecargaInternacionalTelefono(Transaccion tx) {}

	@Override public void consultarCompraPinInternacional(Transaccion tx) {}

	@Override public void consultarRecargaInternacionalTarjeta(Transaccion tx) {}

	@Override public void consultarRecargaInternacionalTelefono(Transaccion tx) {}

	@Override public void recargarInternacionalTarjeta(Transaccion tx) {}

	@Override public void recargarInternacionalTelefono(Transaccion tx) {}
...
//			this.buscarServicio();
	...
	public int removerPendientePorNumeroTransacion(Long numeroTransaccion) {
		try {
			List<TransaccionFullCarga> ticketsPendientes=this.pendientesPorTicket(numeroTransaccion, TipoOperacion.RECARGA );
			for(TransaccionFullCarga t:ticketsPendientes) {
				ClaroTrbListener listener=(ClaroTrbListener) this.pendientes.get(t);
				this.removeAndCancelPendiente(listener);
			}
	...
	public void guardarTrama(long producto, String trama,long numeroTransaccion,long codigoMovimiento,long transacId){
		AdminFullCargaLocal adminTran=(AdminFullCargaLocal) this.registro.getAdminTransaccion(producto);
	...
	private HttpRequest buildHttpRequest(TransaccionFullCarga tx) throws JAXBException
	{
		...
		builderC.append("<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:net=\"net.sigma.h2h.ws\">\r"); 
		builderC.append("<soapenv:Header/>\r");  
		builderC.append("<soapenv:Body>\r"); 		
		if(tx.getTipoOperacion()==TipoOperacion.COMPRAR_PIN_INTERNET)
		{
			...
			builderC.append("<codigoProducto>"+getIDProducto(tx.getCodigoProveedorProducto())+"</codigoProducto>\r"); 
			builderC.append("<referenciaOperacion>"+tx.getNumeroTransaccion()+"</referenciaOperacion>\r"); 
			builderC.append("<fechaHoraOperacion>"+Utilitaria.formatFecha(new Date(), "yyyy-MM-dd")+"T"+Utilitaria.formatFecha(new Date(), "hh:mm:ss")+"</fechaHoraOperacion>\r"); 
			builderC.append("<parametrosOperacion>\r"); 
			builderC.append("<indice>"+1+"</indice>\r"); 
			builderC.append("<valor>"+editNumber(tx.getReferenciaCliente())+"</valor>\r"); 
			builderC.append("</parametrosOperacion>\r"); 
			builderC.append("<parametrosOperacion>\r"); 
			builderC.append("<indice>"+2+"</indice>\r"); 
			builderC.append("<valor>"+tx.getValorContable()+"</valor>\r"); 
			builderC.append("</parametrosOperacion>\r"); 
			builderC.append("</net:venta>\r"); 

		}
		//cambio 09/15
		else if (tx.getTipoOperacion()==TipoOperacion.RECARGA) {
			builderC.append("<net:venta>\r"); 
			builderC.append("<tpv>"+datConWsdl.getCanal()+"</tpv>\r"); 
			builderC.append("<claveTpv>"+datConWsdl.getTerminal()+"</claveTpv>\r"); 
			builderC.append("<codigoProducto>"+getIDProducto(tx.getCodigoProveedorProducto())+"</codigoProducto>\r"); 
