...
public class Datos {
	...
	public boolean insertarTransaccional(Connection Conn, Long codigoMovimiento, Long numTransaccion,
			java.util.Date fecha, java.util.Date fechaRecibidaColaPeticion, String descripcion, Long codigoCliente,
			Long codigoUsuario, BigDecimal valorContable, String referenciaCliente, String mvmParametroXmlIn,
			String identificadorHost, String referenciaProveedor, String mvmParametroXmlOut, Long codigoBodega,
			Long codigoProveedorProducto, int codigoEstado, int codigoProceso) throws Exception {
		boolean ban = false;
		PreparedStatement sentenciaSql3;

		String cadena3 = "INSERT INTO public.conexion( " + "cnx_codigo_movimiento, " + // 1
	...
	public boolean actualizarTransaccionalEnvioRecarga(Connection Conn, Date fechaEnvioCompraPinInternet,
		...
		boolean ban = false;
		String cadena3 = "UPDATE public.conexion  SET " + "cnx_fecha_envio_cmp_pin_internet = ?, "
				+ "cnx_estado_fk = ?, " + "cnx_proceso_fk = ?, " + "cnx_transac_id = ? " + "WHERE "
				+ "(cnx_num_transaccion = ?)";
	...
	public boolean insertarTransaccionalDetalleEnvioRecarga(Connection Conn, Long codigoMovimiento, Long numTransaccion,
			Long transacId, String numeroTransaccion, Date fechaTransaccion, BigDecimal valor,
			String fechaConciliacionStr) throws Exception {
		boolean ban = false;

		String cadena3 = "INSERT INTO  public.conexion_detalle ( " + "cnx_det_codigo_movimiento, " + // 1
	...
	// corregir
	public boolean insertarTransaccionalDetalleEnvioAnulacion(Connection Conn, Long codigoMovimiento,
			Long numeroTransaccion, Date fechaEnvio, Long transacId, String valor) throws Exception {
		boolean ban = false;

		String cadena3 = "INSERT INTO  public.conexion_detalle ( " + "cnx_det_codigo_movimiento, "
				+ "cnx_det_num_transaccion, " + "cnx_det_transac_id, " + "cnx_det_numero_transaccion, "
	...
	public boolean actualizarTransaccionalRespuesta(Connection Conn, int codigoEstado, int codigoProceso,
			Date fechaRespuesta, String referencia, String descripcionProveedor, Long numeroTransaccion,
			Long codigoMovimiento, Long transacId, String codigoRespuesta) throws Exception {
		boolean ban = false;
		PreparedStatement sentenciaSql3;

		String cadena3 = "UPDATE " + "public.conexion " + "SET " + "cnx_estado_fk = ?, " + "cnx_proceso_fk = ?, "
				+ "cnx_referencia_proveedor = ?, " + "cnx_descripcion_proveedor = ?, "
				+ "cnx_fecha_respuesta_comprar_pin_internet = ?, " + "cnx_codigo_respuesta = ? " + "WHERE "
				+ "(cnx_codigo_movimiento = ?)AND " + "(cnx_num_transaccion = ?)";
	...
	public List<String> consultarTramasPorNumeroTransaccion(Connection Conn, Long numeroTransaccion) throws Exception {
		...
		String cadena = "Select " + "trn.cnx_det_transac_id," + "trn.cnx_det_etapa_trn_st, "
				+ "trn.cnx_det_trn_requerimiento, " + "trn.cnx_det_trn_respuesta " +
				/*
				 * "trn.cnx_det_reverso_requerimiento, " + "trn.cnx_det_reverso_respuesta " +
				 */
				"FROM " + "public.conexion_detalle as trn " + "WHERE " + "(trn.cnx_det_num_transaccion = ?)"
		...
		while (rs.next()) {
			...
			// String tramaReversoRequerimiento =
			// rs.getString("cnx_det_reverso_requerimiento");
	...
	public boolean insertarTramaTransaccionRequerimiento(Connection Conn, Long numeroTransaccion, Long codigoMovimiento,
			Long transacId, String tramaTrnRequerimiento) throws Exception {

		boolean ban = false;

		PreparedStatement sentenciaSql3;

		String cadena3 = "UPDATE " + "public.conexion_detalle " + "SET " + "cnx_det_trn_requerimiento = ? " + "WHERE "
				+ "(cnx_det_num_transaccion = ?) AND " + "(cnx_det_codigo_movimiento = ?) AND "
				+ "(cnx_det_transac_id = ?)";
	...
	public boolean insertarTramaTransaccionRespuesta(Connection Conn, Long numeroTransaccion, Long codigoMovimiento,
			Long transacId, String tramaTrnRespuesta) throws Exception {

		boolean ban = false;

		PreparedStatement sentenciaSql3;

		String cadena3 = "UPDATE " + "public.conexion_detalle " + "SET " + "cnx_det_trn_respuesta = ?" + "WHERE "
				+ "(cnx_det_num_transaccion = ?) AND " + "(cnx_det_codigo_movimiento = ?) AND "
				+ "(cnx_det_transac_id = ?)";
	...
	public boolean actualizarTransaccionalDetalleRespuesta(Connection Conn, Long numeroTransaccion,
			Long codigoMovimiento, Long transacId, String codigoRetorno, String mensajeRetorno, String tramaReq,
			String tramaResp, String tipoProducto) throws Exception {
		boolean ban = false;
		PreparedStatement sentenciaSql3;

		String cadena3 = "UPDATE " + "public.conexion_detalle " + "SET " + "cnx_det_codigo_retorno = ? ," + // 1
				"cnx_det_mensaje_retorno = ? ," + // 2
				"cnx_det_trama_requerimiento = ?, " + // 3
				"cnx_det_trama_respuesta = ?, " + // 4
				"cnx_det_tipo_producto = ? " + // 5
				"WHERE " + "(cnx_det_num_transaccion = ?) AND " + // 6
	...
	public boolean actualizarTransaccionalEstadoPorNumeroticketFinal(Connection Conn, int codigoEstado,
			int codigoProceso, String referencia, Long numTransaccion, Long codigoMovimiento,
			String descripcionProveedor) throws Exception {
		boolean ban = false;
		PreparedStatement sentenciaSql3;

		String cadena3 = "UPDATE " + "public.conexion  " + "SET " + "cnx_estado_fk = ? ," + "cnx_proceso_fk = ? ,"
				+ "cnx_referencia_proveedor = ? ," + "cnx_descripcion_proveedor = ? " + "WHERE "
				+ "(cnx_num_transaccion = ?)AND " + "(cnx_codigo_movimiento = ?)";
	...
	public boolean actualizarTransaccionAnulacionEstadoPorNumeroticketFinal(Connection Conn, int codigoEstado,
			int codigoProceso, Long numTransaccion, Long codigoMovimiento, String descripcionProveedor)
			throws Exception {
		boolean ban = false;
		PreparedStatement sentenciaSql3;

		String cadena3 = "UPDATE " + "public.conexion  " + "SET " + "cnx_estado_fk = ? ," + // 1
				"cnx_proceso_fk = ? ," + // 2
				"cnx_descripcion_proveedor = ? " + "WHERE " + "(cnx_num_transaccion = ?)AND "
	...
	public Map<Long, PropiedadesProveedorProducto> consultarPropiedadesProductosProveedor(Connection Conn)
			throws Exception {
		PreparedStatement sentenciaSql;
		ResultSet rs = null;
	...
	public boolean actualizarTransaccionalEnvioAnulacion(Connection Conn,
			...
			int codigoEstado, int codigoProceso, long transacId, Long codigoMovimiento, Long numTransaccion,
		...
		String cadena3 = "UPDATE " + "public.conexion " + "SET " + "cnx_estado_fk = ?, " + // 1
				...
				"WHERE " + "(cnx_codigo_movimiento = ?) AND " + // 5
	...
	public boolean actualizarAnulacionNoRealizada(Connection Conn,
			...
			int codigoEstado, int codigoProceso, long transacId, Long codigoMovimiento, Long numTransaccion,
			Date fechaRespuestaAnulacionPinInternet, String codigoRespuesta, String descripcioError) throws Exception {
		boolean ban = false;
		PreparedStatement sentenciaSql3;
		...
		String cadena3 = "UPDATE " + "public.conexion " + "SET " + "cnx_estado_fk = ?, " + // 1
				...
				"WHERE " + "(cnx_codigo_movimiento = ?) AND " + // 7
	...
	public List<Transaccional> consultarTransaccionalPendientes(Connection Conn) throws Exception {
		...
		// int estado9 = 78;//Recibida del MDB para proceso de consulta de la compra de
		// pin de internet
		...
		// int estado8 = 65;//Recibida del MDB para proceso de Anulacion de la compra de
		// pin

		PreparedStatement sentenciaSql;
		ResultSet rs = null;
		String cadena = "SELECT " + "trn.cnx_codigo_movimiento, " + // 1
				...
				"FROM " + "public.conexion as trn " + "WHERE " + "((trn.cnx_estado_fk = ?) OR " + // 1
		...
		while (rs.next()) {
			...
			Date fechaRecibidaColaPeticionRecarga = rs
					.getTimestamp("cnx_fecha_recibida_cola_peticion_cmp_pin_internet"); // 18
	...
	public Long actualizarNumConsulta(Connection Conn, Long numConsulta, Long codEmpresa) throws Exception {
		...
		String cadena3 = "UPDATE " + " public.configuracion  " + "SET " + "cnf_num_transaccion_consulta = ? " + // 1
	...
	public Long obtenerNumConsulta(Connection Conn, Long codEmpresa) throws Exception {
		...
		String cadena = " UPDATE public.configuracion  SET cnf_num_transaccion_consulta =(SELECT cnf_num_transaccion_consulta  From configuracion WHERE cnf_cod_empresa = ?) +1 "
				+ " WHERE cnf_cod_empresa = ? " + " RETURNING  cnf_num_transaccion_consulta ";
		sentenciaSql = Conn.prepareStatement(cadena);
	...
	public Long consultarUltimoTransacid(Connection Conn) throws Exception {
		...
		PreparedStatement sentenciaSql;
		ResultSet rs = null;
		String cadena = "SELECT " + "MAX(tdt.cnx_det_transac_id) AS transac_id " + "FROM "
	...
	public Date consultarUltimaFechaConciliacion(Connection Conn) throws Exception {
		...
		PreparedStatement sentenciaSql;
		ResultSet rs = null;
		String cadena = "SELECT " + "MAX(tdt.cnd_fecha_conciliacion) AS fecha_conciliacion "
	...
	public boolean insertarRegistroDeConcilidacion(Connection Conn, String fecha, String nombreArchivo,
			Long transaccionConsilidadas, Long transaccionNoConcilidada) throws Exception {
		boolean ban = false;

		String cadena3 = "INSERT INTO  public.conciliacion ( " + "ccc_fecha, " + // 1
	...
	public boolean borrarRegistroDeConcilidacion(Connection Conn, String nombreArchivo) throws Exception {
		boolean ban = false;

		String cadena3 = "DELETE from transacciones_no_conciliadas " + "WHERE tcn_nombre_archivo_conciliacion='"
				+ nombreArchivo + "' ";
	...
	public Map<Long, Empresa> listaEmpresa(Connection Conn) throws Exception {
		...
		String cadena = "SELECT  id_empresa from empresa ";
	...
	public ConfiguracionCorreo listarConfiguracionCorreo(Connection Conn) throws Exception {
		...
		String cadena = "SELECT "
				+ "cfc_user_correo,  cfc_password_correo, cfc_correos_destinatarios, cfc_correos_copia  " + "FROM "
